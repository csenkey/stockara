...
**Status:** Partially done. Candidate scoring now separates scored evidence from neutral/context-only signals: neutral news momentum remains available as prompt/reviewer context but no longer inflates opportunity scores, context-only rows are excluded from opportunity/risk totals, one-day price/volume moves require same-direction multi-day or source-backed confirmation before they affect ranking, sector-relative scoring now uses 5-session and 20-session relative returns versus sector ETFs with noise and history gates, earnings/dividend event scoring now requires sufficient reaction history or explicit catalyst news before affecting ranking, and live provider enrichment now treats options, analyst, insider, and institutional availability as context unless the provider data contains a clear directional aggregate. Remaining work includes richer valuation/fundamental evidence and invalidation evidence.
