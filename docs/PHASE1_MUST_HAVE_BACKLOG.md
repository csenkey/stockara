...
**Status:** Complete for the current P1 scope. Candidate scoring now separates scored evidence from neutral/context-only signals: neutral news momentum remains available as prompt/reviewer context but no longer inflates opportunity scores, context-only rows are excluded from opportunity/risk totals, one-day price/volume moves require same-direction multi-day or source-backed confirmation before they affect ranking, sector-relative scoring now uses 5-session and 20-session relative returns versus sector ETFs with noise and history gates, earnings/dividend event scoring now requires sufficient reaction history or explicit catalyst news before affecting ranking, live provider enrichment now treats options, analyst, insider, and institutional availability as context unless the provider data contains a clear directional aggregate, fundamental/valuation enrichment now scores only coherent provider-backed quality, weakness, or valuation extremes, and analysis/review prompts now carry signal-derived invalidation checks for actionable recommendations.
...
**Status:** Done for the old near-real-time P1 scope, then revised for quota-conscious development. As of 2026-07-28, the direct EventBridge news collection schedule runs three times per day instead of every 15 minutes so NewsAPI free development/test quota is not exhausted. News ticker classification still filters to the active ticker universe when available, uses word-boundary fallback extraction, suppresses common-word short tickers unless provider-tagged or strongly disambiguated, and stores classification confidence/provenance metadata. News source availability reporting separates provider request health from article count, records failed/skipped/zero-article sources, redacts provider secrets from error reasons, and makes CI warnings name failed configured sources.

**Gap:** Completed for low-frequency prefetching. The new Daily Pipeline Stability plan should make the daily orchestrator responsible for final pre-analysis news readiness, repair, and degraded-publication decisions.

**Why it matters:** Timely and correctly classified news is central to catalyst analysis.

**Required outcome:**

- Align the deployed news schedule with the once-daily analysis requirement and provider quota budgets.
...
**Implemented policy:** The watchlist seed custom resource now syncs source-backed static metadata when stock metadata already exists, keyed by a hash of `data/watchlist_seed.csv` so CSV changes retrigger the custom resource on deploy. The sync updates static fields and sell-alert watch flags, creates missing stock metadata rows, refreshes the sell-alert config record, preserves live collection fields such as latest price/date markers, reports created/missing/changed/unchanged/invalid/inactive/out-of-scope counts, and can be invoked manually through `scripts/seed_watchlist.py`, `mode=sync_static_metadata`, or the `Sync Watchlist Metadata Now` GitHub Actions workflow. Phase 1 analysis already reads stored business context into neutral company context for recommendation cards/prompts rather than treating it as scored market evidence.
